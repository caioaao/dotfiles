...
if (!apiKey) {
	...
	console.error("Get your API key at: https://kagi.com/settings?p=api");
	process.exit(1);
}
...
async function fetchKagiResults(query, numResults) {
	const params = new URLSearchParams({
		q: query,
		limit: Math.min(numResults, 20).toString(),
	});
	...
	if (data.data) {
		for (const item of data.data) {
			// t=0 means organic search result
...
// Main
try {
	const results = await fetchKagiResults(query, numResults);
