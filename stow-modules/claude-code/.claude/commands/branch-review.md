...
Base: `$1` - if empty, use `main` (or whatever `gt parent` reports for stacked branches in repos managed with Graphite).
...
3. Delegate codebase context-gathering to sub-agents (Task tool) when useful, so the orchestrator stays lean.
...
Focus areas (arguments beyond `$1`): $ARGUMENTS
