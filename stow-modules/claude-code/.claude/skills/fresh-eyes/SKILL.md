---
name: fresh-eyes
description: Critical review of designs, plans, reasoning, and implementations. Inline tension-based self-review for every task; fresh-context critics via sub-agents (Task tool) for high-stakes work. Use before presenting a proposal, before executing a plan, after long solo reasoning, on hard-to-reverse decisions, and before declaring work done.
---
...
The critic runs via the Task tool (`general-purpose`) with a fresh context -
that is the point. Write the task self-contained: inline the material under
review, absolute paths, and constraints; it sees none of your conversation.
Instruct critics to stay read-only (read, grep, glob only - no edits, no
state-changing commands) so they verify claims against the repo without
acting.
...
- **Different model** - hardest calls: rerun the same review from a session
  on a stronger or different-family model; cross-model disagreement is real
  signal.
