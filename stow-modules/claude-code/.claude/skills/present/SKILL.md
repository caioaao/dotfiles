...
If the source is a file, resolve it to an absolute path. If the material lives only in the conversation, the sub-agent cannot see it - inline it in the task, faithfully and complete. Don't trim to save space; sub-agent tasks handle large payloads fine.

## Delegate first (context hygiene)

Reading the source end-to-end and authoring HTML burns a lot of context. If you have the Task tool, do NOT do this inline. Instead:

1. Spawn a sub-agent with a task like:
   > Read `~/.claude/skills/present/SKILL.md` and follow it exactly. Source: `<absolute path, or the material itself, inlined>`. Work from `<cwd>`. Extra steering: `<caller steering, if any>`.
2. Relay the sub-agent's report (created/refreshed, `file://` URL, chosen angle) to the user verbatim.

If you don't have the Task tool (e.g. you *are* the sub-agent), follow the rest of this file yourself.
